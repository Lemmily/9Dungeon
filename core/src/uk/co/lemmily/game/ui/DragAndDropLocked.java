 ...
 */
public class DragAndDropLocked {
    ...
    ObjectMap<Source, DragListenerLocked> sourceListeners = new ObjectMap();
...
//    @Override
    public void addSource (final Source source) {
        DragListenerLocked listener = new DragListenerLocked() {
            public void dragStart (InputEvent event, float x, float y, int pointer) {
                ...
                float tx = getTouchDownX();
                float ty = getTouchDownY();
                if (tx > ty) {

                } else {

                }
                payload = source.dragStart(event, tx, ty, pointer);
            ...
            public void drag (InputEvent event, float x, float y, int pointer) {
                ...
                float actorY;
                if (xAxis) {
                    actorX = event.getStageX() + dragActorX;
                    actorY = event.getStageY() + actor.getY();
                } else {
                    actorX = event.getStageX() + actor.getX();
                    actorY = event.getStageY() + dragActorY - actor.getHeight();
                }
    ...
    public void removeSource (Source source) {
        DragListenerLocked dragListener = sourceListeners.remove(source);
    ...
    public void clear () {
        targets.clear();
        for (ObjectMap.Entry<Source, DragListenerLocked> entry : sourceListeners.entries())
     ...
     * touch focus listener, eg when the source is inside a {@link com.badlogic.gdx.scenes.scene2d.ui.ScrollPane} with flick scroll enabled. */
